    ...
    for i in range(recur_layers):
        if (i == 0):
            rnn = GRU(units, activation='relu', return_sequences=True,
                      implementation=2, name='gru'+str(i))(input_data)
        else:
            rnn = GRU(units, activation='relu', return_sequences=True,
                                implementation=2, name='gru'+str(i))(bn_rnn)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(
        GRU(units, activation='relu', return_sequences=True, implementation=2, name='rnn'),
...
def final_model(input_dim, units, recur_layers, filters, kernel_size, conv_stride,
                conv_border_mode, dropout_rate=0.2, output_dim=29):
    """ Build a deep network for speech 
    """
    # Main acoustic input
    input_data = Input(name='the_input', shape=(None, input_dim))
    # TODO: Specify the layers in your network
    # add convotionnal layer
    conv_1d = Conv1D(filters, kernel_size, 
                     strides=conv_stride, 
                     padding=conv_border_mode,
                     activation='relu',
